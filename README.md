...
- 支持手动添加、编辑和删除作者信息
- 支持作者码 + 卡号总位数、显式起止卡号和规则优先级，正确处理前导零及重叠区间
- 支持按 CDB 来源、`name`/`desc`/`str1..str16` 字段和值维护高优先级文本规则，并独立指定作者归属或人工系列分组
- 作者识别优先级：管理员文本规则 > 管理员卡号区间 > CDB署名 > strings.conf保守兜底
- 卡片详情显示判定来源，规则或卡库变更会自动使作者榜缓存失效
- 详细规则见 [作者归属判定文档](docs/AUTHOR_RESOLUTION.md)
